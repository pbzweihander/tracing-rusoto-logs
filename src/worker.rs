...
) -> Result<Option<String>, RusotoLogsError> {
...
async fn rusoto_worker_loop(
    ...
    while let Some(log_events) = event_chunk.recv().await {
        ...
        match put_log_events(client.as_ref(), request).await {
            Ok(next_sequence_token) => sequence_token = next_sequence_token,
            Err(error) => eprintln!("Failed to deliver logs: {}", error),
        }
    }
}
